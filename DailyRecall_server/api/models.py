...
class User_Post(models.Model):
    subject = models.CharField(
        max_length=255
    )  # What broad area was learned ('Math', 'Programming')
    ...
    content = (
        models.TextField()
    )  # Optional: a more detailed explanation of what was learned

    author = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_post"
    )  # Link the post to a user
    date_posted = models.DateField(
        auto_now_add=True
    )  # Automatically store the date the post was made
